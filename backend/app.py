...
from PIL import Image
import base64
from agents import OBBModule, TOCRAgent, TOCRPollingAgent
from fastapi.middleware.cors import CORSMiddleware
from io import BytesIO
from utils import convert_htm_to_excel
from openpyxl.drawing.image import Image as ExcelImage
...
sheet = client.open_by_key(sheet_id).worksheet("Anthropic_Loop")
sheet_cons = client.open_by_key(sheet_id).worksheet("Cons.")
...
polling_agent = TOCRPollingAgent(system_prompt_updated=open("./system_prompt.txt", 'r').read(), job_collection=job_collection)
obb = OBBModule('./dynamic_quantized_21.onnx')
...
async def polling_extract(
    ...
    print("batch size: ", len(batch_request))
    ...
    if os.path.exists(temp_dir):
        try:
            ...
            print(f"Error cleaning up temporary directory {temp_dir}: {str(cleanup_err)}")
                ...
                for table in tables:
                    for gen_table in table['html_code']:
                        ...
                        if not table_df.empty:
                            ...
                            print(f"skipping a empty table: {excel_file_per_table}")
